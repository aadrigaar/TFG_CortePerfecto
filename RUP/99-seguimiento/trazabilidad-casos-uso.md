...
| UC-02 | Consultar servicios y precios | `/api/services` | `serviceController.js`, `HomePage.jsx`, `ServiceCard.jsx` | `serviceCatalog.js`, `serviceCatalogService.js`, `serviceApi` | Colección `servicios` sincronizada desde catálogo | Build frontend |
...
| UC-04 | Abrir chat y enviar mensaje | Chat widget | `ChatWidget.jsx`, `chatController.js` | `chatRequestService.js`, `chatRuleService.js`, `lmStudioService.js` | Historial saneado en cliente y servidor | `chatHardening.test.js`, build frontend |
...
| UC-09 | Modificar reserva activa por chat | `POST /api/chat` | `chatController.js` | `bookingFlowService.js`, `appointmentService.js` | `appointments`, vinculadas a `conversationId` | `bookingFlowService.test.js`, `appointmentService.test.js` |
...
| UC-13 | Crear cita manual | `POST /api/appointments` | `appointmentController.js`, `AppointmentForm.jsx` | `serviceApi`, `appointmentService.createAppointment()` | `appointments`, catálogo de `servicios` | `appointmentService.test.js` |
...
| Mantener respuesta útil si LM Studio no está disponible. | `chatController.js`, `chatRuleService.js` | `chatHardening.test.js`, prueba HTTP de contingencia |
| No modificar una cita desde otra conversación. | `appointmentService.js`, `bookingFlowService.js` | `appointmentService.test.js` |
| No reutilizar fecha u hora anterior tras una entrada inválida. | `bookingFlowService.js` | `bookingFlowService.test.js` |
